...
<template>
  <div class="flex h-[calc(100vh-64px)] overflow-hidden bg-slate-50 dark:bg-slate-950 font-sans" v-if="issue">
    
    <!-- Left Pane: Leads -->
    <div class="w-1/3 xl:w-1/4 border-r border-slate-200 dark:border-slate-800 overflow-y-auto px-6 py-6 bg-white dark:bg-slate-900 shadow-sm z-10 flex flex-col gap-10">
      ...
      <div v-else class="flex flex-col gap-8">
        ...
        <section v-if="newLeads.length > 0">
           <div class="flex items-center gap-3 mb-4">
             <div class="w-6 h-6 rounded flex items-center justify-center text-white font-bold text-xs bg-primary-500">N</div>
             <h2 class="text-sm font-bold tracking-tight text-slate-900 dark:text-white uppercase uppercase">New Leads</h2>
           </div>
           
           <draggable :list="newLeads" group="leads" item-key="_id" ghost-class="opacity-40" class="flex flex-col gap-3 min-h-[50px]">
             <template #item="{ element: lead }">
               <div class="cursor-pointer">
                 <LeadCard :lead="lead" :existing-issues="existingIssues" />
               </div>
             </template>
           </draggable>
        </section>
        ...
        <section v-for="section in groupedSections" :key="section.id">
           <div class="flex items-center gap-3 mb-4">
             <div class="w-6 h-6 rounded flex items-center justify-center text-white font-bold text-xs" :class="section.id === 'idea-pool' ? 'bg-amber-500' : 'bg-slate-700 dark:bg-slate-600'">
               {{ section.id === 'idea-pool' ? '★' : '#' }}
             </div>
             <h2 class="text-sm font-bold tracking-tight text-slate-900 dark:text-white uppercase">{{ section.title }}</h2>
             <span class="ml-auto text-xs font-semibold text-slate-400 tabular-nums">{{ section.leads.length }}</span>
           </div>
           
           <draggable :list="section.leads" group="leads" item-key="_id" ghost-class="opacity-40" class="flex flex-col gap-3 min-h-[50px]">
             <template #item="{ element: lead }">
               <div class="cursor-pointer">
                 <LeadCard :lead="lead" :existing-issues="existingIssues" />
               </div>
        ...
        <div v-if="newLeads.length === 0 && groupedSections.length === 0" class="text-center py-10">
            <p class="text-sm text-slate-500">All available leads are assigned!</p>
        </div>
      </div>
    </div>
    
    <!-- Right Pane: Magazine Editor Grid -->
    <div class="w-2/3 xl:w-3/4 overflow-y-auto relative">
      ...
      <div class="sticky top-0 z-20 px-8 py-5 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-md border-b border-slate-200/50 dark:border-slate-800/50 flex justify-between items-center shadow-sm">
          ...
          <div class="flex items-center gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2 rounded-xl shadow-sm">
             ...
             <UInput type="number" v-model.number="totalPages" @change="onTotalPagesChange" class="w-24 text-center font-bold" min="1" />
          </div>
      </div>
      ...
      <div class="px-8 py-8">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
           <!-- Single Page -->
           <div v-for="page in issue.pages" :key="page.pageNumber"
                :class="[
               ...
               <div class="absolute top-0 left-0 w-full h-10 flex items-center px-4 justify-between border-b border-slate-100 dark:border-slate-800 backdrop-blur-sm bg-white/50 dark:bg-slate-900/50 rounded-t-2xl">
                    ...
                    <div class="opacity-0 group-hover:opacity-100 transition-opacity z-10 w-24">
                        ...
                        <select class="w-full text-xs font-semibold border border-slate-300 dark:border-slate-600 rounded-lg py-1 px-2 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 focus:outline-none cursor-pointer"
                                @change="setPageRows(page, Number(($event.target as HTMLSelectElement).value))">
                            <option v-for="n in 4" :key="n" :value="n" :selected="page.rows.length === n">{{ n }} Row{{ n>1 ? 's': '' }}</option>
                        </select>
                    </div>
               </div>
               
               <div class="flex-1 flex flex-col gap-2 mt-2">
                   <div v-for="(row, idx) in page.rows" :key="row.id" 
                       ...
                       <draggable v-model="row.items" group="leads" item-key="_id" class="flex-1 w-full h-full flex flex-col relative z-0" @add="onRowAdd($event, row)" ghost-class="!h-full flex-1 opacity-50">
                               ...
                               <div class="w-full h-full flex-1 bg-white dark:bg-slate-800 flex items-center justify-between px-4 py-2 group/item relative z-10 transition-colors">
                                   ...
                                   <template v-else>
                                       <div class="flex items-center gap-3 z-20 w-full overflow-hidden">
                                           <div class="w-2 h-full absolute left-0 top-0 bottom-0 bg-primary-500"></div>
                                           <span class="font-bold text-sm text-slate-800 dark:text-slate-100 truncate pl-2">{{ element.title || element.company }}</span>
                                       </div>
                                   </template>
                                   
                                   <!-- Delete Assignment Button -->
                                   <button aria-label="Delete assignment" class="z-30 text-slate-400 hover:text-red-500 hidden group-hover/item:flex items-center justify-center absolute right-2 opacity-100 bg-white dark:bg-slate-800 shadow-sm p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 transition" @click="onDeleteItem(row)">
                           ...
                           <template #footer>
                                <!-- Empty State -->
                                <div v-if="row.items.length === 0" class="absolute inset-0 w-full h-full flex items-center justify-between px-3 text-slate-400 z-0 pointer-events-none">
                                    <span class="text-xs font-medium uppercase tracking-wider text-slate-400/70">Drop lead</span>
                                    <div class="pointer-events-auto">
                                        <UButton size="xs" variant="outline" color="gray" class="bg-white/50 dark:bg-slate-800/50 font-bold hover:bg-slate-100 hover:text-red-600 transition" @click="onAddAd(row)">Ads</UButton>
                                    </div>
                                </div>
                           </template>
                       </draggable>
